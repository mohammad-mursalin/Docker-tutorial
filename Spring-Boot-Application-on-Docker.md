...
This document provides a list of basic Docker CLI commands for running a Spring Boot aplication on docker.
...
```bash
docker cp target/docker-demo.jar <container_name>:/tmp
```
> This copies the `docker-demo.jar` into the container’s /tmp directory.
...
docker exec <container_name> ls /tmp
```
> The `docker-demo.jar` file will be available in addition to the existing content.


###  Commit the Container to Create a New Docker Image

```bash
docker commit <container_name> mursalin/docker-demo:v1
```
> Creates a new Docker image named `mursalin/docker-demo` with tag `v1` from the current container state.
...
docker images
```
> Verifies that `mursalin/docker-demo:v1` image has been created successfully.


### Default Behavior: JShell

When running mursalin/docker-demo:v1, the container defaults to JShell:

```bash
docker run mursalin/docker-demo:v1
```
...
```bash
docker commit --change='CMD ["java", "-jar", "/tmp/docker-demo.jar"]' <container_name> mursalin/docker-demo:v2
```
...
```bash
docker run mursalin/docker-demo:v2
```
...
```bash
docker run -p 8081:8081 mursalin/docker-demo:v2
```
